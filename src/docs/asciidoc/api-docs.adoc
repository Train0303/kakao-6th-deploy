...
** body
*** email(string) : 이메일 형식을 만족해야 합니다.
*** password(string) : 영문, 숫자, 특수문자(공백 제외) 8~20자를 만족해야 합니다.
*** username(string) : 8~45자를 만족해야합니다.
...
** body
*** optionId(number) : 0보다 큰 수를 입력해야 합니다.
*** quantity(number) : 0보다 큰 수를 입력해야 합니다.
...
==== 실패 요청 예시 : 빈 장바구니
include::{snippets}/{cart}/find-all_test_fail_empty_cart/http-request.adoc[]

==== 정상 응답 예시 : 빈 장바구니
include::{snippets}/{cart}/find-all_test_fail_empty_cart/response-body.adoc[]
...
*** Authorization : Bearer {token}
** body
*** cartId(number) : 0보다 큰 수를 입력해야 합니다.
*** quantity(number) : 0보다 큰 수를 입력해야 합니다.
